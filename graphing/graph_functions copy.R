...
mds_plot <- function(meta_data, mds, k, iter, seed, plot_dir) {
  ...
  orgin_plot <- ggplot(mds, aes(x = MDS1, y = MDS2,
    ...
    scale_color_manual(name = "Sample Time",
                       labels = c("Ind.",
                                    "Pal.",
                                    "Pre."),
...
bee_plot <- function(iterative_data, plot_dir) {
  ...
  bee <- ggplot(melted_data, aes(x=Var2, y=value)) +
    geom_beeswarm(size = 3, dodge.width = 1, aes(color=Var2)) +
    stat_boxplot(size = 1, geom = "errorbar") +
    geom_boxplot(size = 1, color = safe_colors[14], width = 0.1,
                 outlier.shape = NA) +
