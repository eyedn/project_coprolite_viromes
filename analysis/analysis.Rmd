...
base_dir <- "~/Documents/ucla/research/project_coprolite_viromes/analysis/"
...
set.seed(seed = 123)
data_dir <- "~/Documents/ucla/research/project_coprolite_viromes_NOT_CODE/data"
figs_dir <- "~/Documents/ucla/research/project_coprolite_viromes_NOT_CODE/figures"
max_iter <- 1000
subset_top <- 100
...
lifestyle_counts["temp_to_lyt_+1", ] <- 
  (lifestyle_counts["temperate", ] + 1) / (lifestyle_counts["virulent", ] + 1)
...
vir_ec_optimal
ggsave(filename = paste0(figs_dir, "/metabolism/silhouette.svg"),
         plot = vir_ec_optimal,
         height = 7,
         width = 7)
...
determine CI of p-vals of heatmap genes
```{r metabolic gene analysis: p-val CIs}
library(boot)

# all metabolic genes in heatmap
all_met_pvals <- as.numeric(read.csv(
  file = paste0(data_dir, "/all_met_pvals.txt"),
  header = FALSE)[1, ])
set.seed(1234)
all_met_boot <- boot(
  all_met_pvals, 
...
all_met_ci <- boot.ci(
  ...
  conf = 0.95,
  type = "perc")
cat("---------- ALL METABOLIC GENES ----------\n")
cat(paste0("median: ", all_met_ci[2], "\n"))
cat(paste0("95% CI (last 2 values): ", all_met_ci[4], "\n"))

# red cluster metabolic genes in heatmap
red_met_pvals <- as.numeric(read.csv(
  file = paste0(data_dir, "/red_cluster_met_pvals.txt"),
  header = FALSE)[1, ])
set.seed(1234)
red_met_boot <- boot(
  red_met_pvals, 
...
red_met_ci <- boot.ci(
  ...
  type = "perc")
cat("\n---------- RED METABOLIC GENES ----------\n")
cat(paste0("median: ", red_met_ci[2], "\n"))
cat(paste0("95% CI (last 2 values): ", red_met_ci[4], "\n"))
```
...
determine CI of p-vals of heatmap genes
```{r r vfdb alignment: p-val CIs}
library(boot)

# all vf genes in heatmap
all_vf_pvals <- as.numeric(read.csv(
  file = paste0(data_dir, "/all_vf_pvals.txt"),
  header = FALSE)[1, ])
set.seed(1234)
all_vf_boot <- boot(
  all_vf_pvals, 
...
all_vf_ci <- boot.ci(
  ...
  type = "perc")
cat("---------- ALL VF GENES ----------\n")
cat(paste0("median: ", all_vf_ci[2], "\n"))
cat(paste0("95% CI (last 2 values): ", all_vf_ci[4], "\n"))

# blue cluster vf genes in heatmap
blue_met_pvals <- as.numeric(read.csv(
  file = paste0(data_dir, "/blue_cluster_vf_pvals.txt"),
  header = FALSE)[1, ])
set.seed(1234)
blue_vf_boot <- boot(
  blue_met_pvals, 
...
blue_vf_ci <- boot.ci(
  ...
  type = "perc")
cat("\n---------- BLUE VF GENES ----------\n")
cat(paste0("median: ", blue_vf_ci[2], "\n"))
cat(paste0("95% CI (last 2 values): ", blue_vf_ci[4], "\n"))
```
